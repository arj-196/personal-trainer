...
def test_plan_writes_comparison_files_for_multiple_models(tmp_path, monkeypatch) -> None:
    workspaces_root = tmp_path / "workspaces"
    workspace = workspaces_root / "athlete"
    monkeypatch.setattr("personal_trainer.cli.WORKSPACES_ROOT", workspaces_root)
    _install_stub_ollama(monkeypatch, day_count=3)
    ...
    assert "Generated by: openai/gpt-5.4-mini" in openai_plan.read_text(encoding="utf-8")
